...
void KFParticlePerformanceBase::CreateHistos(string histoDir, TDirectory* outFile)
{
  ...
  //             float xMax[nFitQA/2] = {2.,2.,5.,0.3,0.3,0.3,0.03,0.03};
    ...
    gDirectory->cd("PrimaryVertexQA");
    {
      ...
      float xMin[nHistosPVParam] = {-1., -1., -100.,  0,   -0.5,    0.,   -0.5,    0., 0., -0.01, -0.01, -0.01, -0.01, -0.01, 0.};
      float xMax[nHistosPVParam] = { 1.,  1.,  100., 10, 1000.5, 1000., 1000.5, 1000., 1.,  1.01,  1.01,  1.01,  1.01,  1.01, 100.};
                                                          ...
                                                          int iPart, bool drawZR)
{
  ...
  TString parName2D[nHistoPartParam2D] = {"y-p_{t}", "Z-R", "Armenteros", "y-m_{t}", "M-p_{t}"};
  ...
  histoParameters2D[iPart][4] = new TH2F(parName2D[4].Data(),parTitle2D[4].Data(),
					nBins[0],xMin[0], xMax[0],     //M
					nBins[2],xMin[2],xMax[2]);     //pt
  histoParameters2D[iPart][4]->GetXaxis()->SetTitle("M [GeV/c^{2}]");
  histoParameters2D[iPart][4]->GetYaxis()->SetTitle("p_{t} [GeV/c]");
